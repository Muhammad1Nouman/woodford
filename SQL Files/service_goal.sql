DELIMITER $$
CREATE DEFINER=`pbiRefresh`@`%` PROCEDURE `service_goal`()
BEGIN
with monthly_goals_with_percentage AS (
    SELECT 
        mp.year,
        mp.month,
        mp.service_sale AS value
...
goal_into_percent AS (
    SELECT 
        ...
        (yg.service * mp.value) AS service_sale,
        (yg.service_stop * mp.value) as service_stop
...
daily_values as(
    SELECT 
        ...
        gd.year,
        gd.month,
        gd.service_sale / gd.days_in_month AS service_sale,
        gd.service_stop / gd.days_in_month as service_stop
...
SELECT 
    date,
    "goal" as type,
    sum(service_sale) AS service_sale,
    sum(service_stop) as service_stop
FROM daily_values
group by 1
ORDER BY 1 DESC;
END$$
